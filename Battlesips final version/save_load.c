...
// If not found, returns false
{
    ...
    if ((file = fopen("players.txt", "r")) == NULL) // if we cant open the file for some reason
    {
...
// - name: Name of the new player (chosen by user).
{
    ...
    if ((file = fopen("players.txt", "a")) == NULL)
    {
...
bool isTopPlayer(const Player* p)
{
    ...
    while (fscanf(fp, "%s %d %d", tempPlayer.name, &tempPlayer.rank, &tempPlayer.score) == 3)
    {
...
void updatePlayerInFile(Player* p)
{
    ...
    // Open files
    if ((originalFile = fopen("players.txt", "r")) == NULL ||
