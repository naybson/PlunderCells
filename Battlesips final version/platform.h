...
#define PLATFORM_H
  ...
  static inline void               platform_pause(void)               { system("pause"); }
#else
  #include <termios.h>
  #include <unistd.h>
  #include <time.h>
  ...
  void               platform_pause_raw(void);   /* restore canonical mode for scanf */
  void               platform_resume_raw(void);  /* re-enable raw mode after scanf   */
  ...
  /* system("pause") replacement */
  #include <stdio.h>
  static inline void platform_pause(void) {
